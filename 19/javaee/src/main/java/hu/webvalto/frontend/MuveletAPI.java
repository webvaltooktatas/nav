...
import hu.webvalto.frontend.beans.BetetKivetMuvelet;
...
public class MuveletAPI implements Serializable {
    ...
    public void feldolgozas() {
        if (szamla.getMuvelet().equals("egyenleg")) {
            ...
            Set<ConstraintViolation<Szamla>> violations = validator.validate(szamla, BetetKivetMuvelet.class);
            for (ConstraintViolation<Szamla> violation : violations) {
                FacesContext facesContext = FacesContext.getCurrentInstance();
                facesContext.addMessage("muveletiForm", new FacesMessage(violation.getMessage()));
            }
